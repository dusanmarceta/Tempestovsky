...
@jit(nopython=True)
def compute_facet_areas(vertices):
    n = vertices.shape[0]
    areas = np.zeros(n)
    for i in range(n):
        v0 = vertices[i, 0]
...
def sun_direction(time_from_aphelion, simulation):
     ...
     true_anomaly = ecc2true(ecc_anomaly, simulation.ecc)
     
     r = simulation.a_au * (1 - simulation.ecc**2) / (1 + simulation.ecc * np.cos(true_anomaly))

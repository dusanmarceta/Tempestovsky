...
from tumbling_final_2 import compute_tumbling_dynamics_2
...
if __name__ == "__main__":
    ...
    sim_ne = compute_tumbling_dynamics_2(dt = -time_step, timesteps = N_steps, y0 = y0,
                                        I = np.array([I1, I2, I3]), 
                                        r_inertial = r_inertial, 
                                        lambda_L_deg = config["lambda_L"], beta_L_deg = config["beta_L"])
...
long_radi = np.rad2deg(np.arctan2(y_radi, x_radi))
lat_radi = np.rad2deg(np.arcsin(np.clip(z_radi, -1.0, 1.0)))  # clip radi numeričke stabilnosti

long_ne = np.rad2deg(np.arctan2(y_ne, x_ne))
lat_ne = np.rad2deg(np.arcsin(np.clip(z_ne, -1.0, 1.0)))  # clip radi numeričke stabilnosti
...
plt.plot(time, r_radi.T[0])
plt.plot(time, r_ne.T[0])

plt.figure()
plt.title('y', fontsize = 24)

plt.plot(time, r_radi.T[1])
plt.plot(time, r_ne.T[1])

plt.figure()
plt.title('z', fontsize = 24)

plt.plot(time, r_radi.T[2])
plt.plot(time, r_ne.T[2])


plt.figure()
plt.title('long', fontsize = 24)
plt.plot(time, long_radi)
plt.plot(time, long_ne)

plt.figure()
plt.title('lat', fontsize = 24)
plt.plot(time, lat_radi)
plt.plot(time, lat_ne)

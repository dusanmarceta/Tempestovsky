...
from src.model.insolation import calculate_insolation_one_step
...
@jit(nopython=True)
def calculate_temperatures_one_step(temperatures, layer_temperatures, insolation, 
                                    visible_facets_list, view_factors_list, 
                                    const1, const2, const3, self_heating_const,
                                    n_layers, include_self_heating):
    """
    Izračunava temperaturno polje asteroida za jedan vremenski korak dt.
    Početne temperature slojeva se uzimaju iz layer_temperatures[:, prev_column, :].
    """
    ...
    prev_column = 0
    current_column = 1  # Koristi drugi „column“ za ažuriranje
    ...
    for i in range(n_facets):
        ...
        insolation_term = insolation[i] * const1
        
        re_emitted_radiation_term = -const2 * (prev_temp**4)
        
        secondary_radiation_term = 0.0
        if include_self_heating:
            secondary_radiation_term = self_heating_const * np.sum(
                layer_temperatures[:, prev_column, 0][visible_facets_list[i]]**4 * view_factors_list[i]
            )

        conducted_heat_term = const3 * (prev_temp_layer1 - prev_temp)
        

        new_temp = prev_temp + insolation_term + re_emitted_radiation_term + conducted_heat_term + secondary_radiation_term
        ...
        for layer in range(1, n_layers - 1):
            ...
            layer_temperatures[i, current_column, layer] = prev_layer + const3 * (
                prev_layer_plus - 2 * prev_layer + prev_layer_minus
            )
...
class YarkovskySolver(TemperatureSolver):
    ...
    def solve(self, thermal_data, shape_model, simulation, config):
        ...
        insolation_step = thermal_data.insolation[:, 0]
...
#        F=2/3*simulation.emissivity*const.sigma_sb.value / const.c.value * np.sum((T[surface_cells][:, None])**4 * surface_normals * surface_areas[:, None], axis=0)
        ...
        # time_from_aphelion_s = 0
        

        # while time_from_aphelion_s <= orbital_period:
        #         # Površinska temperatura

            
        #     insolation_step = calculate_insolation_one_step(
        #         thermal_data,
        #         shape_model,
        #         simulation,
        #         config,
        #         time_from_aphelion_s
        #     )
        ...
        #     print(insolation_step)
        #     print('****************************************')
        #     print('****************************************')
            
        #     current_temperature = calculate_temperatures_one_step(
        #         thermal_data.temperatures,
        #         thermal_data.layer_temperatures,
        #         insolation_step,
        #         thermal_data.visible_facets,
        ...
        #     time_from_aphelion_s += simulation.delta_t
...
# =============================================================================
#         F=2/3*simulation.emissivity*const.sigma_sb.value / const.c.value * np.sum((current_day_temperature[:, -1]**4 
#                                                                                    * thermal_data.facet_areas)[:, None] 
#                     * thermal_data.normals, axis=0)
#         
#         
# 
#         t_trans = np.cross(simulation.sunlight_direction, np.array([0, 0, 1]))
#         
#         
# 
#         # Transfersal thermal force (Yarkovsky force)
#         B = np.dot(F, simulation.sunlight_direction)
#         
# =============================================================================
...
#        plt.contourf(current_day_temperature[0])
